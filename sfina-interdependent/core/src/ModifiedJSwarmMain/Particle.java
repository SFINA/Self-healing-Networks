...
public class Particle {
    ...
    public void allocate(int dimension) {
        int dim = setDimension(dimension);
        ...
        for (int i = 0; i < position.length; i++) {
            //System.out.println("error error pos"+position.length);
    ...
    public void init(double maxPosition[], double minPosition[], double maxVelocity[], double minVelocity[]) {
        for (int i = 0; i < position.length; i++) {
            //System.out.println("error position "+position.length);
            if (Double.isNaN(maxPosition[i])) {
    ...
    public Object selfFactory() {
        ...
        try {
            cons = cl.getConstructor((Class[]) new Class[]{int.class});
        ...
        try {
            return cons.newInstance((Object[]) new Object[]{getDimension()});
    ...
    public int setDimension(int newDim) {
        return newDim;
        //int dim = newDimension;
    }
