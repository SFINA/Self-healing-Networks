...
public class Swarm implements Iterable<Particle> {
    ...
    public static int DEFAULT_NUMBER_OF_PARTICLES = 100;
    public static double DEFAULT_PARTICLE_INCREMENT = 0.9;
    public static double VELOCITY_GRAPH_FACTOR = 10.0;

    public ArrayList<Double> laggedRanX = new ArrayList<Double>();
    public ArrayList<Double> laggedRanY = new ArrayList<Double>();
    ...
    /**
     * Array for best position
     */
    public ArrayList<Double> arrayBestPosition = new ArrayList<Double>();
    ...
    public Swarm(int numberOfParticles, Particle sampleParticle, FitnessFunction fitnessFunction) {
        ...
        particleUpdate = new ParticleUpdateRandomByParticle(sampleParticle);
    ...
    public void init() {
        ...
        for (int i = 0; i < numberOfParticles; i++) {
            ...
            //particles[i].custominit()[0]=laggedRanX.get(i);
            //particles[i].custominit()[1]=laggedRanY.get(i);
        }
     ...
     * @param maxPosition
     */
    public void setMaxPosition(double [] maxPosition1) { //here give an array to make it more general as the dimensions increases
        if (sampleParticle == null) {
            throw new RuntimeException("Need to set sample particle before calling this method (use Swarm.setSampleParticle() method)");
        }
        int dim = sampleParticle.getDimension();
        ...
        //for (int i = 0; i < dim; i++)
        for (int i=0;i<dim;i++){
            this.maxPosition[i] = maxPosition1[i];
            //this.minPosition[1] = minPosition1[1];
        }
    }
     ...
     * @param minPosition
     */
    public void setMinPosition(double [] minPosition1) {//double minPosition1, double minPosition2
        if (sampleParticle == null) {
            throw new RuntimeException("Need to set sample particle before calling this method (use Swarm.setSampleParticle() method)");
        }
        int dim = sampleParticle.getDimension();
        ...
        //for (int i = 0; i < dim; i++)
        //this.minPosition[0] = minPosition1;
        //this.minPosition[1] = minPosition2;
        for (int i=0;i<dim;i++){
            this.minPosition[i] = minPosition1[i];
            //this.minPosition[1] = minPosition1[1];
        }
    }
    ...
    public ArrayList<Double> givebestPosition() {

        for (int i = 0; i < bestPosition.length; i++) {
            arrayBestPosition.add(bestPosition[i]);
        }
        return arrayBestPosition;
    }
    ...
    //lagged fibionacci random number generator
    ...
    public void appendRan() {
        ini();
        for (int i = 0; i < numberOfParticles; i++) {
            laggedRanX.add((maxPosition[0] - minPosition[0])*genrand() + minPosition[0]);
            laggedRanY.add((maxPosition[1] - minPosition[1])*genrand() + minPosition[1]);
            //System.out.println((50-0)*genrand()+0); //max = 50, min =0
        }
    }
}
